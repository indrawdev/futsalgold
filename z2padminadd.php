...
// Create page object
$z2padmin_add = new cz2padmin_add();
$Page =& $z2padmin_add;

// Page init
$z2padmin_add->Page_Init();

// Page main
$z2padmin_add->Page_Main();
?>
...
// Create page object
var z2padmin_add = new ew_Page("z2padmin_add");

// page properties
z2padmin_add.PageID = "add"; // page ID
z2padmin_add.FormID = "fz2padminadd"; // form ID
var EW_PAGE_ID = z2padmin_add.PageID; // for backward compatibility

// extend page with ValidateForm function
z2padmin_add.ValidateForm = function(fobj) {
...
z2padmin_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
z2padmin_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
z2padmin_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
z2padmin_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $z2padmin->TableCaption() ?><br><br>
<a href="<?php echo $z2padmin->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$z2padmin_add->ShowMessage();
?>
<form name="fz2padminadd" id="fz2padminadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return z2padmin_add.ValidateForm(this);">
<p>
...
class cz2padmin_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $z2padmin->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$z2padmin->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $z2padmin->CurrentAction = "C"; // Copy record
		  } else {
		    $z2padmin->CurrentAction = "I"; // Display blank record
		...
		switch ($z2padmin->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$z2padmin->SendEmail = TRUE; // Send email on add success
		...
		$z2padmin->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($z2padmin->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$bValidUser = $Security->IsValidUserID($z2padmin->Username->CurrentValue);
			if (!$bValidUser) {
				$sUserIdMsg = str_replace("%c", CurrentUserID(), $Language->Phrase("UnAuthorizedUserID"));
				$sUserIdMsg = str_replace("%u", $z2padmin->Username->CurrentValue, $sUserIdMsg);
		...
		$bInsertRow = $z2padmin->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($z2padmin->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$z2padmin->ID->setDbValue($conn->Insert_ID());
			$rsnew['ID'] = $z2padmin->ID->DbValue;

			// Call Row Inserted event
			$z2padmin->Row_Inserted($rsnew);
		}
		return $AddRow;
	}

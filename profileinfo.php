...
class cprofile extends cTable {
	...
	function __construct() {
		...
		// ID
		$this->ID = new cField('profile', 'profile', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// Kode
		$this->Kode = new cField('profile', 'profile', 'x_Kode', 'Kode', '`Kode`', '`Kode`', 200, -1, FALSE, '`Kode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Kode'] = &$this->Kode;

		// NamaToko
		$this->NamaToko = new cField('profile', 'profile', 'x_NamaToko', 'NamaToko', '`NamaToko`', '`NamaToko`', 200, -1, FALSE, '`NamaToko`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['NamaToko'] = &$this->NamaToko;

		// Pemilik
		$this->Pemilik = new cField('profile', 'profile', 'x_Pemilik', 'Pemilik', '`Pemilik`', '`Pemilik`', 200, -1, FALSE, '`Pemilik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Pemilik'] = &$this->Pemilik;

		// Alamat
		$this->Alamat = new cField('profile', 'profile', 'x_Alamat', 'Alamat', '`Alamat`', '`Alamat`', 201, -1, FALSE, '`Alamat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Alamat'] = &$this->Alamat;

		// Kota
		$this->Kota = new cField('profile', 'profile', 'x_Kota', 'Kota', '`Kota`', '`Kota`', 200, -1, FALSE, '`Kota`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Kota'] = &$this->Kota;

		// Telepon
		$this->Telepon = new cField('profile', 'profile', 'x_Telepon', 'Telepon', '`Telepon`', '`Telepon`', 200, -1, FALSE, '`Telepon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Telepon'] = &$this->Telepon;

		// Email
		$this->_Email = new cField('profile', 'profile', 'x__Email', 'Email', '`Email`', '`Email`', 200, -1, FALSE, '`Email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Email'] = &$this->_Email;

		// Foto
		$this->Foto = new cField('profile', 'profile', 'x_Foto', 'Foto', '`Foto`', '`Foto`', 205, -1, TRUE, '`Foto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Foto'] = &$this->Foto;

		// Serial
		$this->Serial = new cField('profile', 'profile', 'x_Serial', 'Serial', '`Serial`', '`Serial`', 200, -1, FALSE, '`Serial`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Serial'] = &$this->Serial;

		// KeyCode
		$this->KeyCode = new cField('profile', 'profile', 'x_KeyCode', 'KeyCode', '`KeyCode`', '`KeyCode`', 200, -1, FALSE, '`KeyCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['KeyCode'] = &$this->KeyCode;

		// Waktu
		$this->Waktu = new cField('profile', 'profile', 'x_Waktu', 'Waktu', '`Waktu`', 'DATE_FORMAT(`Waktu`, \'%d/%m/%Y\')', 135, 7, FALSE, '`Waktu`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Waktu->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['Waktu'] = &$this->Waktu;

		// Stamp
		$this->Stamp = new cField('profile', 'profile', 'x_Stamp', 'Stamp', '`Stamp`', 'DATE_FORMAT(`Stamp`, \'%d/%m/%Y\')', 135, 7, FALSE, '`Stamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Stamp->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['Stamp'] = &$this->Stamp;

		// Mobile
		$this->Mobile = new cField('profile', 'profile', 'x_Mobile', 'Mobile', '`Mobile`', '`Mobile`', 3, -1, FALSE, '`Mobile`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Mobile->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
   ...
   // Common render codes
		...
		$this->KeyCode->ViewCustomAttributes = "";
		...
		if (!empty($this->Foto->Upload->DbValue)) {
			$this->Foto->HrefValue = "profile_Foto_bv.php?ID=" . $this->ID->CurrentValue;
			$this->Foto->LinkAttrs["target"] = "_blank";
			if ($this->Export <> "") $this->Foto->HrefValue = ew_ConvertFullUrl($this->Foto->HrefValue);
		} else {
			$this->Foto->HrefValue = "";
		}
		$this->Foto->HrefValue2 = "profile_Foto_bv.php?ID=" . $this->ID->CurrentValue;
		...
		// KeyCode
		$this->KeyCode->LinkCustomAttributes = "";
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->NamaToko->Exportable) $Doc->ExportCaption($this->NamaToko);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->NamaToko->Exportable) $Doc->ExportField($this->NamaToko);

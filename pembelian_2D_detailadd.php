...
$pembelian_2D_detail_add = NULL; // Initialize page object first

class cpembelian_2D_detail_add extends cpembelian_2D_detail {
	...
	// Page object name
	var $PageObjName = 'pembelian_2D_detail_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "pembelian_2D_detailview.php")
	...
	function LoadDefaultValues() {
		...
		$this->Harga_Beli->CurrentValue = NULL;
		$this->Harga_Beli->OldValue = $this->Harga_Beli->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->Harga_Beli->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->Harga_Beli->FldCaption()));
	...
	function ValidateForm() {
		...
		if (!$this->Harga_Beli->FldIsDetailKey && !is_null($this->Harga_Beli->FormValue) && $this->Harga_Beli->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->Harga_Beli->FldCaption());
		}
		if (!ew_CheckInteger($this->Harga_Beli->FormValue)) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$sMasterFilter = $this->SqlMasterFilter_pembelian_2D_master();
			if (strval($this->IDM->CurrentValue) <> "" &&
				$this->getCurrentMasterTable() == "pembelian_2D_master") {
				$sMasterFilter = str_replace("@ID@", ew_AdjustSql($this->IDM->CurrentValue), $sMasterFilter);
			} else {
				$sMasterFilter = "";
			}
			if ($sMasterFilter <> "") {
				$rsmaster = $GLOBALS["pembelian_2D_master"]->LoadRs($sMasterFilter);
		...
		// Harga Beli
		$this->Harga_Beli->SetDbValueDef($rsnew, $this->Harga_Beli->CurrentValue, 0, FALSE);
	...
	function Page_Redirecting(&$url) {
	    ...
	    //$url = "your URL";
	     ...
	     if ($_POST['x_IDM']){
	         echo "<meta http-equiv=refresh content=0;URL=AppHitungPembelianDetail.php?id=".$_POST[x_IDM]."&go=".$url.">";
	         exit;
	        } 
	}                                                 
...
// Create page object
if (!isset($pembelian_2D_detail_add)) $pembelian_2D_detail_add = new cpembelian_2D_detail_add();

// Page init
$pembelian_2D_detail_add->Page_Init();

// Page main
$pembelian_2D_detail_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pembelian_2D_detail_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var pembelian_2D_detail_add = new ew_Page("pembelian_2D_detail_add");
pembelian_2D_detail_add.PageID = "add"; // Page ID
var EW_PAGE_ID = pembelian_2D_detail_add.PageID; // For backward compatibility

// Form object
var fpembelian_2D_detailadd = new ew_Form("fpembelian_2D_detailadd");

// Validate form
fpembelian_2D_detailadd.Validate = function() {
...
fpembelian_2D_detailadd.Form_CustomValidate = 
...
fpembelian_2D_detailadd.ValidateRequired = true;
<?php } else { ?>
fpembelian_2D_detailadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpembelian_2D_detailadd.Lists["x_Kode"] = {"LinkField":"x_Kode","Ajax":null,"AutoFill":true,"DisplayFields":["x_Kode","x_Nama_Barang","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $pembelian_2D_detail_add->ShowPageHeader(); ?>
<?php
$pembelian_2D_detail_add->ShowMessage();
?>
<form name="fpembelian_2D_detailadd" id="fpembelian_2D_detailadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="pembelian_2D_detail">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_pembelian_2D_detailadd" class="table table-bordered table-striped">
<?php if ($pembelian_2D_detail->Kode->Visible) { // Kode ?>
	<tr id="r_Kode"<?php echo $pembelian_2D_detail->RowAttributes() ?>>
		<td><span id="elh_pembelian_2D_detail_Kode"><?php echo $pembelian_2D_detail->Kode->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $pembelian_2D_detail->Kode->CellAttributes() ?>><span id="el_pembelian_2D_detail_Kode" class="control-group">
<?php $pembelian_2D_detail->Kode->EditAttrs["onchange"] = "ew_AutoFill(this); " . @$pembelian_2D_detail->Kode->EditAttrs["onchange"]; ?>
<select data-field="x_Kode" id="x_Kode" name="x_Kode"<?php echo $pembelian_2D_detail->Kode->EditAttributes() ?>>
<?php
...
<script type="text/javascript">
fpembelian_2D_detailadd.Lists["x_Kode"].Options = <?php echo (is_array($pembelian_2D_detail->Kode->EditValue)) ? ew_ArrayToJson($pembelian_2D_detail->Kode->EditValue, 1) : "[]" ?>;
</script>
<?php
...
<?php if ($pembelian_2D_detail->Nama_Barang->Visible) { // Nama Barang ?>
	<tr id="r_Nama_Barang"<?php echo $pembelian_2D_detail->RowAttributes() ?>>
		<td><span id="elh_pembelian_2D_detail_Nama_Barang"><?php echo $pembelian_2D_detail->Nama_Barang->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $pembelian_2D_detail->Nama_Barang->CellAttributes() ?>><span id="el_pembelian_2D_detail_Nama_Barang" class="control-group">
<input type="text" data-field="x_Nama_Barang" name="x_Nama_Barang" id="x_Nama_Barang" size="30" maxlength="255" placeholder="<?php echo $pembelian_2D_detail->Nama_Barang->PlaceHolder ?>" value="<?php echo $pembelian_2D_detail->Nama_Barang->EditValue ?>"<?php echo $pembelian_2D_detail->Nama_Barang->EditAttributes() ?>>
</span><?php echo $pembelian_2D_detail->Nama_Barang->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($pembelian_2D_detail->Satuan->Visible) { // Satuan ?>
	<tr id="r_Satuan"<?php echo $pembelian_2D_detail->RowAttributes() ?>>
		<td><span id="elh_pembelian_2D_detail_Satuan"><?php echo $pembelian_2D_detail->Satuan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $pembelian_2D_detail->Satuan->CellAttributes() ?>><span id="el_pembelian_2D_detail_Satuan" class="control-group">
<input type="text" data-field="x_Satuan" name="x_Satuan" id="x_Satuan" size="30" maxlength="255" placeholder="<?php echo $pembelian_2D_detail->Satuan->PlaceHolder ?>" value="<?php echo $pembelian_2D_detail->Satuan->EditValue ?>"<?php echo $pembelian_2D_detail->Satuan->EditAttributes() ?>>
</span><?php echo $pembelian_2D_detail->Satuan->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($pembelian_2D_detail->Harga_Beli->Visible) { // Harga Beli ?>
	<tr id="r_Harga_Beli"<?php echo $pembelian_2D_detail->RowAttributes() ?>>
		<td><span id="elh_pembelian_2D_detail_Harga_Beli"><?php echo $pembelian_2D_detail->Harga_Beli->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $pembelian_2D_detail->Harga_Beli->CellAttributes() ?>><span id="el_pembelian_2D_detail_Harga_Beli" class="control-group">
<input type="text" data-field="x_Harga_Beli" name="x_Harga_Beli" id="x_Harga_Beli" size="30" placeholder="<?php echo $pembelian_2D_detail->Harga_Beli->PlaceHolder ?>" value="<?php echo $pembelian_2D_detail->Harga_Beli->EditValue ?>"<?php echo $pembelian_2D_detail->Harga_Beli->EditAttributes() ?>>
</span><?php echo $pembelian_2D_detail->Harga_Beli->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($pembelian_2D_detail->Jumlah->Visible) { // Jumlah ?>
	<tr id="r_Jumlah"<?php echo $pembelian_2D_detail->RowAttributes() ?>>
		<td><span id="elh_pembelian_2D_detail_Jumlah"><?php echo $pembelian_2D_detail->Jumlah->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $pembelian_2D_detail->Jumlah->CellAttributes() ?>><span id="el_pembelian_2D_detail_Jumlah" class="control-group">
<input type="text" data-field="x_Jumlah" name="x_Jumlah" id="x_Jumlah" size="30" placeholder="<?php echo $pembelian_2D_detail->Jumlah->PlaceHolder ?>" value="<?php echo $pembelian_2D_detail->Jumlah->EditValue ?>"<?php echo $pembelian_2D_detail->Jumlah->EditAttributes() ?>>
</span><?php echo $pembelian_2D_detail->Jumlah->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<?php if (strval($pembelian_2D_detail->IDM->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_IDM" id="x_IDM" value="<?php echo ew_HtmlEncode(strval($pembelian_2D_detail->IDM->getSessionValue())) ?>">
<?php } ?>
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
</form>
<script type="text/javascript">
fpembelian_2D_detailadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$pembelian_2D_detail_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$pembelian_2D_detail_add->Page_Terminate();
?>

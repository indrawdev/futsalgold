...
mysql_select_db($database_Konek, $Konek);
$query_RB = "SELECT Sum(`Total HJ`) as SubTotal FROM `penjualan - detail` WHERE `IDM`=".$row_RA['ID'];
$RB = mysql_query($query_RB, $Konek) or die(mysql_error());
$row_RB = mysql_fetch_assoc($RB);
$totalRows_RB = mysql_num_rows($RB);

mysql_select_db($database_Konek, $Konek);
$query_RC = "SELECT * FROM `penjualan - detail` WHERE `IDM`=".$row_RA['ID'];
$RC = mysql_query($query_RC, $Konek) or die(mysql_error());
$row_RC = mysql_fetch_assoc($RC);
$totalRows_RC = mysql_num_rows($RC);

mysql_select_db($database_Konek, $Konek);
$query_RD = "SELECT Sum(`Total HP`) as HP FROM `penjualan - detail` WHERE `IDM`=".$row_RA['ID'];
...
if ($row_RA['Dibayar']>=$row_RB['SubTotal']){
$Sisa = 0;
}else{
$Sisa = $row_RB['SubTotal']-$row_RA['Dibayar'];
}
$HP = $row_RD['HP'];
...
$updateSQLM = "UPDATE `penjualan - master` SET `SisaDibayar`=$Sisa, `Total HP`=$HP, `Total HJ`=$SubTotal, `Grand Total`=$GrandTotal, `Laba`=$Laba WHERE ID=".$_GET['id'];                                                                        
...
echo "<meta http-equiv=refresh content=0;URL=".FUrlRef().">";
}
?>
<?php

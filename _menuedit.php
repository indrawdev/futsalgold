...
class cp_menu_edit extends c_menu {
	...
	function ValidateForm() {
		...
		if (in_array("menusub1", $DetailTblVar) && $GLOBALS["menusub1"]->DetailEdit) {
		...
		if ($rs->EOF) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("menusub1", $DetailTblVar) && $GLOBALS["menusub1"]->DetailEdit) {
						if (!isset($GLOBALS["menusub1_grid"])) $GLOBALS["menusub1_grid"] = new cmenusub1_grid(); // Get detail page object
						$EditRow = $GLOBALS["menusub1_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("menusub1", $DetailTblVar)) {
				...
				if ($GLOBALS["menusub1_grid"]->DetailEdit) {
					$GLOBALS["menusub1_grid"]->CurrentMode = "edit";
					$GLOBALS["menusub1_grid"]->CurrentAction = "gridedit";
...
<input type="hidden" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($_menu->ID->CurrentValue) ?>">
<?php
	if (in_array("menusub1", explode(",", $_menu->getCurrentDetailTable())) && $menusub1->DetailEdit) {
?>

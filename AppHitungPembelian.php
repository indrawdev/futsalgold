...
mysql_select_db($database_Konek, $Konek);
$query_RB = "SELECT Sum(`Total HP`) as SubTotal FROM `pembelian - detail` WHERE `IDM`=".$row_RA['ID'];
$RB = mysql_query($query_RB, $Konek) or die(mysql_error());
$row_RB = mysql_fetch_assoc($RB);
$totalRows_RB = mysql_num_rows($RB);

mysql_select_db($database_Konek, $Konek);
$query_RC = "SELECT * FROM `pembelian - detail` WHERE `IDM`=".$row_RA['ID'];
...
$GrandTotal = $row_RB['SubTotal'];
}      
	  
$updateSQLM = "UPDATE `pembelian - master` SET `SisaDibayar`=$Sisa, `Total HP`=$SubTotal, `Grand Total`=$GrandTotal WHERE ID=".$_GET['id'];                                                                        

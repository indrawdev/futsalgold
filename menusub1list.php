...
$menusub1_list = NULL; // Initialize page object first

class cmenusub1_list extends cmenusub1 {
	...
	// Page object name
	var $PageObjName = 'menusub1_list';

	// Grid form hidden field names
	var $FormName = 'fmenusub1list';
	...
	function __construct() {
		...
		$this->AddUrl = "menusub1add.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "menusub1delete.php";
		$this->MultiUpdateUrl = "menusub1update.php";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "_menu") {
			global $_menu;
			$rsmaster = $_menu->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("_menulist.php"); // Return to master page
			} else {
				$_menu->LoadListRowValues($rsmaster);
				$_menu->RowType = EW_ROWTYPE_MASTER; // Master row
				$_menu->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupListOptions() {
		...
		// "detail_menusub2"
		$item = &$this->ListOptions->Add("detail_menusub2");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'menusub2') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
		$item->ShowInButtonGroup = FALSE;
		if (!isset($GLOBALS["menusub2_grid"])) $GLOBALS["menusub2_grid"] = new cmenusub2_grid;
	...
	function RenderListOptions() {
		...
		// "detail_menusub2"
		$oListOpt = &$this->ListOptions->Items["detail_menusub2"];
		if ($Security->AllowList(CurrentProjectID() . 'menusub2')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("menusub2", "TblCaption");
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("menusub2list.php?" . EW_TABLE_SHOW_MASTER . "=menusub1&_Menu=" . strval($this->_Menu->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["menusub2_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'menusub2')) {
				$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=menusub2")) . "\">" . $Language->Phrase("MasterDetailEditLink") . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "menusub2";
			}
			if ($links <> "") {
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_menusub2");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=menusub2") . "\">" . $Language->Phrase("AddLink") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["menusub2"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["menusub2"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'menusub2') && $Security->CanAdd());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "menusub2";
		}
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.fmenusub1list, '" . $this->MultiDeleteUrl . "');return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
	...
	function RenderOtherOptions() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fmenusub1list, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@MainMenu@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_menusub1\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_menusub1',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmenusub1list,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "_menu") {
			global $_menu;
			$rsmaster = $_menu->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $ExportDoc->Style;
				$ExportDoc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$_menu->ExportDocument($ExportDoc, $rsmaster, 1, 1);
...
// Create page object
if (!isset($menusub1_list)) $menusub1_list = new cmenusub1_list();

// Page init
$menusub1_list->Page_Init();

// Page main
$menusub1_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$menusub1_list->Page_Render();
?>
...
// Page object
var menusub1_list = new ew_Page("menusub1_list");
menusub1_list.PageID = "list"; // Page ID
var EW_PAGE_ID = menusub1_list.PageID; // For backward compatibility

// Form object
var fmenusub1list = new ew_Form("fmenusub1list");
fmenusub1list.FormKeyCountName = '<?php echo $menusub1_list->FormKeyCountName ?>';

// Validate form
fmenusub1list.Validate = function() {
...
// Check empty row
fmenusub1list.EmptyRow = function(infix) {
	var fobj = this.Form;
...
fmenusub1list.Form_CustomValidate = 
...
fmenusub1list.ValidateRequired = true;
<?php } else { ?>
fmenusub1list.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php if ($menusub1->getCurrentMasterTable() == "" && $menusub1_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $menusub1_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php if (($menusub1->Export == "") || (EW_EXPORT_MASTER_RECORD && $menusub1->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "_menulist.php";
if ($menusub1_list->DbMasterFilter <> "" && $menusub1->getCurrentMasterTable() == "_menu") {
	if ($menusub1_list->MasterRecordExists) {
		if ($menusub1->getCurrentMasterTable() == $menusub1->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($menusub1_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $menusub1_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php include_once "_menumaster.php" ?>
<?php
	}
}
?>
...
if ($menusub1->CurrentAction == "gridadd") {
	...
	$menusub1_list->StartRec = 1;
	$menusub1_list->DisplayRecs = $menusub1->GridAddRowCount;
	$menusub1_list->TotalRecs = $menusub1_list->DisplayRecs;
	$menusub1_list->StopRec = $menusub1_list->DisplayRecs;
} else {
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$menusub1_list->TotalRecs = $menusub1->SelectRecordCount();
	} else {
		if ($menusub1_list->Recordset = $menusub1_list->LoadRecordset())
			$menusub1_list->TotalRecs = $menusub1_list->Recordset->RecordCount();
	}
	$menusub1_list->StartRec = 1;
	if ($menusub1_list->DisplayRecs <= 0 || ($menusub1->Export <> "" && $menusub1->ExportAll)) // Display all records
		$menusub1_list->DisplayRecs = $menusub1_list->TotalRecs;
	if (!($menusub1->Export <> "" && $menusub1->ExportAll))
		$menusub1_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$menusub1_list->Recordset = $menusub1_list->LoadRecordset($menusub1_list->StartRec-1, $menusub1_list->DisplayRecs);
}
$menusub1_list->RenderOtherOptions();
?>
<?php $menusub1_list->ShowPageHeader(); ?>
<?php
$menusub1_list->ShowMessage();
?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<form name="fmenusub1list" id="fmenusub1list" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="menusub1">
<div id="gmp_menusub1" class="ewGridMiddlePanel">
<?php if ($menusub1_list->TotalRecs > 0) { ?>
<table id="tbl_menusub1list" class="ewTable ewTableSeparate">
...
// Render list options
$menusub1_list->RenderListOptions();

// Render list options (header, left)
$menusub1_list->ListOptions->Render("header", "left");
?>
<?php if ($menusub1->_Menu->Visible) { // Menu ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $menusub1->SortUrl($menusub1->_Menu) ?>',1);"><div id="elh_menusub1__Menu" class="menusub1__Menu">
...
<?php if ($menusub1->Link->Visible) { // Link ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $menusub1->SortUrl($menusub1->Link) ?>',1);"><div id="elh_menusub1_Link" class="menusub1_Link">
...
<?php if ($menusub1->Parent->Visible) { // Parent ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $menusub1->SortUrl($menusub1->Parent) ?>',1);"><div id="elh_menusub1_Parent" class="menusub1_Parent">
...
<?php if ($menusub1->No->Visible) { // No ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $menusub1->SortUrl($menusub1->No) ?>',1);"><div id="elh_menusub1_No" class="menusub1_No">
...
<?php if ($menusub1->Akses->Visible) { // Akses ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $menusub1->SortUrl($menusub1->Akses) ?>',1);"><div id="elh_menusub1_Akses" class="menusub1_Akses">
...
$menusub1_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($menusub1->ExportAll && $menusub1->Export <> "") {
	$menusub1_list->StopRec = $menusub1_list->TotalRecs;
} else {

	// Set the last record to display
	if ($menusub1_list->TotalRecs > $menusub1_list->StartRec + $menusub1_list->DisplayRecs - 1)
		$menusub1_list->StopRec = $menusub1_list->StartRec + $menusub1_list->DisplayRecs - 1;
	else
		$menusub1_list->StopRec = $menusub1_list->TotalRecs;
}
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($menusub1_list->FormKeyCountName) && ($menusub1->CurrentAction == "gridadd" || $menusub1->CurrentAction == "gridedit" || $menusub1->CurrentAction == "F")) {
		$menusub1_list->KeyCount = $objForm->GetValue($menusub1_list->FormKeyCountName);
		$menusub1_list->StopRec = $menusub1_list->StartRec + $menusub1_list->KeyCount - 1;
	}
}
$menusub1_list->RecCnt = $menusub1_list->StartRec - 1;
if ($menusub1_list->Recordset && !$menusub1_list->Recordset->EOF) {
	$menusub1_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $menusub1_list->StartRec > 1)
		$menusub1_list->Recordset->Move($menusub1_list->StartRec - 1);
} elseif (!$menusub1->AllowAddDeleteRow && $menusub1_list->StopRec == 0) {
	$menusub1_list->StopRec = $menusub1->GridAddRowCount;
}

// Initialize aggregate
$menusub1->RowType = EW_ROWTYPE_AGGREGATEINIT;
$menusub1->ResetAttrs();
$menusub1_list->RenderRow();
if ($menusub1->CurrentAction == "gridadd")
	$menusub1_list->RowIndex = 0;
if ($menusub1->CurrentAction == "gridedit")
	$menusub1_list->RowIndex = 0;
while ($menusub1_list->RecCnt < $menusub1_list->StopRec) {
	$menusub1_list->RecCnt++;
	if (intval($menusub1_list->RecCnt) >= intval($menusub1_list->StartRec)) {
		$menusub1_list->RowCnt++;
		if ($menusub1->CurrentAction == "gridadd" || $menusub1->CurrentAction == "gridedit" || $menusub1->CurrentAction == "F") {
			$menusub1_list->RowIndex++;
			$objForm->Index = $menusub1_list->RowIndex;
			if ($objForm->HasValue($menusub1_list->FormActionName))
				$menusub1_list->RowAction = strval($objForm->GetValue($menusub1_list->FormActionName));
			elseif ($menusub1->CurrentAction == "gridadd")
				$menusub1_list->RowAction = "insert";
			else
				$menusub1_list->RowAction = "";
		}

		// Set up key count
		$menusub1_list->KeyCount = $menusub1_list->RowIndex;

		// Init row class and style
		$menusub1->ResetAttrs();
		$menusub1->CssClass = "";
		if ($menusub1->CurrentAction == "gridadd") {
			$menusub1_list->LoadDefaultValues(); // Load default values
		} else {
			$menusub1_list->LoadRowValues($menusub1_list->Recordset); // Load row values
		}
		...
		if ($menusub1->CurrentAction == "gridadd" && $menusub1->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$menusub1_list->RestoreCurrentRowFormValues($menusub1_list->RowIndex); // Restore form values
		if ($menusub1->CurrentAction == "gridedit") { // Grid edit
			if ($menusub1->EventCancelled) {
				$menusub1_list->RestoreCurrentRowFormValues($menusub1_list->RowIndex); // Restore form values
			}
			if ($menusub1_list->RowAction == "insert")
				$menusub1->RowType = EW_ROWTYPE_ADD; // Render add
			else
		...
		if ($menusub1->RowType == EW_ROWTYPE_EDIT) // Edit row
			$menusub1_list->EditRowCnt++;

		// Set up row id / data-rowindex
		$menusub1->RowAttrs = array_merge($menusub1->RowAttrs, array('data-rowindex'=>$menusub1_list->RowCnt, 'id'=>'r' . $menusub1_list->RowCnt . '_menusub1', 'data-rowtype'=>$menusub1->RowType));
		...
		if ($menusub1_list->RowAction <> "delete" && $menusub1_list->RowAction <> "insertdelete" && !($menusub1_list->RowAction == "insert" && $menusub1->CurrentAction == "F" && $menusub1_list->EmptyRow())) {
?>
	<tr<?php echo $menusub1->RowAttributes() ?>>
<?php

// Render list options (body, left)
$menusub1_list->ListOptions->Render("body", "left", $menusub1_list->RowCnt);
?>
	<?php if ($menusub1->_Menu->Visible) { // Menu ?>
		<td<?php echo $menusub1->_Menu->CellAttributes() ?>><span id="el<?php echo $menusub1_list->RowCnt ?>_menusub1__Menu" class="control-group menusub1__Menu">
<?php if ($menusub1->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="text" data-field="x__Menu" name="x<?php echo $menusub1_list->RowIndex ?>__Menu" id="x<?php echo $menusub1_list->RowIndex ?>__Menu" size="30" maxlength="100" placeholder="<?php echo $menusub1->_Menu->PlaceHolder ?>" value="<?php echo $menusub1->_Menu->EditValue ?>"<?php echo $menusub1->_Menu->EditAttributes() ?>>
<input type="hidden" data-field="x__Menu" name="o<?php echo $menusub1_list->RowIndex ?>__Menu" id="o<?php echo $menusub1_list->RowIndex ?>__Menu" value="<?php echo ew_HtmlEncode($menusub1->_Menu->OldValue) ?>">
<?php } ?>
...
<?php } ?>
</span><a id="<?php echo $menusub1_list->PageObjName . "_row_" . $menusub1_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php if ($menusub1->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-field="x_ID" name="x<?php echo $menusub1_list->RowIndex ?>_ID" id="x<?php echo $menusub1_list->RowIndex ?>_ID" value="<?php echo ew_HtmlEncode($menusub1->ID->CurrentValue) ?>">
<input type="hidden" data-field="x_ID" name="o<?php echo $menusub1_list->RowIndex ?>_ID" id="o<?php echo $menusub1_list->RowIndex ?>_ID" value="<?php echo ew_HtmlEncode($menusub1->ID->OldValue) ?>">
<?php } ?>
<?php if ($menusub1->RowType == EW_ROWTYPE_EDIT || $menusub1->CurrentMode == "edit") { ?>
<input type="hidden" data-field="x_ID" name="x<?php echo $menusub1_list->RowIndex ?>_ID" id="x<?php echo $menusub1_list->RowIndex ?>_ID" value="<?php echo ew_HtmlEncode($menusub1->ID->CurrentValue) ?>">
<?php } ?>
	<?php if ($menusub1->Link->Visible) { // Link ?>
		<td<?php echo $menusub1->Link->CellAttributes() ?>><span id="el<?php echo $menusub1_list->RowCnt ?>_menusub1_Link" class="control-group menusub1_Link">
<?php if ($menusub1->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="text" data-field="x_Link" name="x<?php echo $menusub1_list->RowIndex ?>_Link" id="x<?php echo $menusub1_list->RowIndex ?>_Link" size="30" maxlength="255" placeholder="<?php echo $menusub1->Link->PlaceHolder ?>" value="<?php echo $menusub1->Link->EditValue ?>"<?php echo $menusub1->Link->EditAttributes() ?>>
<input type="hidden" data-field="x_Link" name="o<?php echo $menusub1_list->RowIndex ?>_Link" id="o<?php echo $menusub1_list->RowIndex ?>_Link" value="<?php echo ew_HtmlEncode($menusub1->Link->OldValue) ?>">
<?php } ?>
<?php if ($menusub1->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<input type="text" data-field="x_Link" name="x<?php echo $menusub1_list->RowIndex ?>_Link" id="x<?php echo $menusub1_list->RowIndex ?>_Link" size="30" maxlength="255" placeholder="<?php echo $menusub1->Link->PlaceHolder ?>" value="<?php echo $menusub1->Link->EditValue ?>"<?php echo $menusub1->Link->EditAttributes() ?>>
<?php } ?>
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($menusub1->Parent->Visible) { // Parent ?>
		<td<?php echo $menusub1->Parent->CellAttributes() ?>><span id="el<?php echo $menusub1_list->RowCnt ?>_menusub1_Parent" class="control-group menusub1_Parent">
<?php if ($menusub1->RowType == EW_ROWTYPE_ADD) { // Add record ?>
...
<input type="hidden" id="x<?php echo $menusub1_list->RowIndex ?>_Parent" name="x<?php echo $menusub1_list->RowIndex ?>_Parent" value="<?php echo ew_HtmlEncode($menusub1->Parent->CurrentValue) ?>">
<?php } else { ?>
<input type="text" data-field="x_Parent" name="x<?php echo $menusub1_list->RowIndex ?>_Parent" id="x<?php echo $menusub1_list->RowIndex ?>_Parent" size="30" maxlength="100" placeholder="<?php echo $menusub1->Parent->PlaceHolder ?>" value="<?php echo $menusub1->Parent->EditValue ?>"<?php echo $menusub1->Parent->EditAttributes() ?>>
<?php } ?>
<input type="hidden" data-field="x_Parent" name="o<?php echo $menusub1_list->RowIndex ?>_Parent" id="o<?php echo $menusub1_list->RowIndex ?>_Parent" value="<?php echo ew_HtmlEncode($menusub1->Parent->OldValue) ?>">
<?php } ?>
<?php if ($menusub1->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<?php if ($menusub1->Parent->getSessionValue() <> "") { ?>
<span<?php echo $menusub1->Parent->ViewAttributes() ?>>
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($menusub1->No->Visible) { // No ?>
		<td<?php echo $menusub1->No->CellAttributes() ?>><span id="el<?php echo $menusub1_list->RowCnt ?>_menusub1_No" class="control-group menusub1_No">
<?php if ($menusub1->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="text" data-field="x_No" name="x<?php echo $menusub1_list->RowIndex ?>_No" id="x<?php echo $menusub1_list->RowIndex ?>_No" size="30" placeholder="<?php echo $menusub1->No->PlaceHolder ?>" value="<?php echo $menusub1->No->EditValue ?>"<?php echo $menusub1->No->EditAttributes() ?>>
<input type="hidden" data-field="x_No" name="o<?php echo $menusub1_list->RowIndex ?>_No" id="o<?php echo $menusub1_list->RowIndex ?>_No" value="<?php echo ew_HtmlEncode($menusub1->No->OldValue) ?>">
<?php } ?>
<?php if ($menusub1->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<input type="text" data-field="x_No" name="x<?php echo $menusub1_list->RowIndex ?>_No" id="x<?php echo $menusub1_list->RowIndex ?>_No" size="30" placeholder="<?php echo $menusub1->No->PlaceHolder ?>" value="<?php echo $menusub1->No->EditValue ?>"<?php echo $menusub1->No->EditAttributes() ?>>
<?php } ?>
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($menusub1->Akses->Visible) { // Akses ?>
		<td<?php echo $menusub1->Akses->CellAttributes() ?>><span id="el<?php echo $menusub1_list->RowCnt ?>_menusub1_Akses" class="control-group menusub1_Akses">
<?php if ($menusub1->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<select data-field="x_Akses" id="x<?php echo $menusub1_list->RowIndex ?>_Akses[]" name="x<?php echo $menusub1_list->RowIndex ?>_Akses[]" multiple="multiple"<?php echo $menusub1->Akses->EditAttributes() ?>>
<?php
...
<input type="hidden" data-field="x_Akses" name="o<?php echo $menusub1_list->RowIndex ?>_Akses[]" id="o<?php echo $menusub1_list->RowIndex ?>_Akses[]" value="<?php echo ew_HtmlEncode($menusub1->Akses->OldValue) ?>">
<?php } ?>
<?php if ($menusub1->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<select data-field="x_Akses" id="x<?php echo $menusub1_list->RowIndex ?>_Akses[]" name="x<?php echo $menusub1_list->RowIndex ?>_Akses[]" multiple="multiple"<?php echo $menusub1->Akses->EditAttributes() ?>>
<?php
...
$menusub1_list->ListOptions->Render("body", "right", $menusub1_list->RowCnt);
?>
	</tr>
<?php if ($menusub1->RowType == EW_ROWTYPE_ADD || $menusub1->RowType == EW_ROWTYPE_EDIT) { ?>
<script type="text/javascript">
fmenusub1list.UpdateOpts(<?php echo $menusub1_list->RowIndex ?>);
</script>
<?php } ?>
<?php
	}
	...
	if ($menusub1->CurrentAction <> "gridadd")
		if (!$menusub1_list->Recordset->EOF) $menusub1_list->Recordset->MoveNext();
}
?>
<?php
	if ($menusub1->CurrentAction == "gridadd" || $menusub1->CurrentAction == "gridedit") {
		$menusub1_list->RowIndex = '$rowindex$';
		$menusub1_list->LoadDefaultValues();

		// Set row properties
		$menusub1->ResetAttrs();
		$menusub1->RowAttrs = array_merge($menusub1->RowAttrs, array('data-rowindex'=>$menusub1_list->RowIndex, 'id'=>'r0_menusub1', 'data-rowtype'=>EW_ROWTYPE_ADD));
		...
		// Render list options
		$menusub1_list->RenderListOptions();
		$menusub1_list->StartRowCnt = 0;
?>
	<tr<?php echo $menusub1->RowAttributes() ?>>
<?php

// Render list options (body, left)
$menusub1_list->ListOptions->Render("body", "left", $menusub1_list->RowIndex);
?>
	<?php if ($menusub1->_Menu->Visible) { // Menu ?>
...
<?php } ?>
<?php if ($menusub1->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $menusub1_list->FormKeyCountName ?>" id="<?php echo $menusub1_list->FormKeyCountName ?>" value="<?php echo $menusub1_list->KeyCount ?>">
<?php echo $menusub1_list->MultiSelectKey ?>
<?php } ?>
<?php if ($menusub1->CurrentAction == "gridedit") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridupdate">
<input type="hidden" name="<?php echo $menusub1_list->FormKeyCountName ?>" id="<?php echo $menusub1_list->FormKeyCountName ?>" value="<?php echo $menusub1_list->KeyCount ?>">
<?php echo $menusub1_list->MultiSelectKey ?>
<?php } ?>
<?php if ($menusub1->CurrentAction == "") { ?>
...
// Close recordset
if ($menusub1_list->Recordset)
	$menusub1_list->Recordset->Close();
?>
<?php if ($menusub1->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($menusub1->CurrentAction <> "gridadd" && $menusub1->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($menusub1_list->Pager)) $menusub1_list->Pager = new cNumericPager($menusub1_list->StartRec, $menusub1_list->DisplayRecs, $menusub1_list->TotalRecs, $menusub1_list->RecRange) ?>
<?php if ($menusub1_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($menusub1_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $menusub1_list->PageUrl() ?>start=<?php echo $menusub1_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($menusub1_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $menusub1_list->PageUrl() ?>start=<?php echo $menusub1_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($menusub1_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $menusub1_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($menusub1_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $menusub1_list->PageUrl() ?>start=<?php echo $menusub1_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($menusub1_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $menusub1_list->PageUrl() ?>start=<?php echo $menusub1_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($menusub1_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $menusub1_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $menusub1_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $menusub1_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($menusub1_list->SearchWhere == "0=101") { ?>
...
<?php
	foreach ($menusub1_list->OtherOptions as &$option)

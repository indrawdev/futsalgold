...
$pembelian_2D_detail_grid = NULL; // Initialize page object first

class cpembelian_2D_detail_grid extends cpembelian_2D_detail {
	...
	// Page object name
	var $PageObjName = 'pembelian_2D_detail_grid';

	// Grid form hidden field names
	var $FormName = 'fpembelian_2D_detailgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["pembelian_2D_detail"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Satuan") && $objForm->HasValue("o_Satuan") && $this->Satuan->CurrentValue <> $this->Satuan->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Harga_Beli") && $objForm->HasValue("o_Harga_Beli") && $this->Harga_Beli->CurrentValue <> $this->Harga_Beli->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Jumlah") && $objForm->HasValue("o_Jumlah") && $this->Jumlah->CurrentValue <> $this->Jumlah->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Total_HP") && $objForm->HasValue("o_Total_HP") && $this->Total_HP->CurrentValue <> $this->Total_HP->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadDefaultValues() {
		...
		$this->Total_HP->CurrentValue = NULL;
		$this->Total_HP->OldValue = $this->Total_HP->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->Harga_Beli->setOldValue($objForm->GetValue("o_Harga_Beli"));
		...
		if (!$this->Total_HP->FldIsDetailKey) {
			$this->Total_HP->setFormValue($objForm->GetValue("x_Total_HP"));
		}
		$this->Total_HP->setOldValue($objForm->GetValue("o_Total_HP"));
	...
	function RestoreFormValues() {
		...
		$this->Total_HP->CurrentValue = $this->Total_HP->FormValue;
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// Total HP
			$this->Total_HP->EditCustomAttributes = "";
			$this->Total_HP->EditValue = ew_HtmlEncode($this->Total_HP->CurrentValue);
			$this->Total_HP->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->Total_HP->FldCaption()));
			...
			// Total HP
			$this->Total_HP->EditCustomAttributes = "";
			$this->Total_HP->EditValue = $this->Total_HP->CurrentValue;
			$this->Total_HP->EditValue = ew_FormatNumber($this->Total_HP->EditValue, 0, -2, -2, -2);
			$this->Total_HP->CellCssStyle .= "text-align: right;";
			$this->Total_HP->ViewCustomAttributes = "";
	...
	function AddRow($rsold = NULL) {
		...
		// Total HP
		$this->Total_HP->SetDbValueDef($rsnew, $this->Total_HP->CurrentValue, 0, FALSE);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "pembelian_2D_master") {
			...
			if ($GLOBALS["pembelian_2D_master"]->EventCancelled) $this->EventCancelled = TRUE;
		}

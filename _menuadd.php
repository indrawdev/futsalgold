...
class cp_menu_add extends c_menu {
	...
	function ValidateForm() {
		...
		if (in_array("menusub1", $DetailTblVar) && $GLOBALS["menusub1"]->DetailAdd) {
			if (!isset($GLOBALS["menusub1_grid"])) $GLOBALS["menusub1_grid"] = new cmenusub1_grid(); // get detail page object
			$GLOBALS["menusub1_grid"]->ValidateGridForm();
		}
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("menusub1", $DetailTblVar) && $GLOBALS["menusub1"]->DetailAdd) {
				$GLOBALS["menusub1"]->Parent->setSessionValue($this->MainMenu->CurrentValue); // Set master key
				if (!isset($GLOBALS["menusub1_grid"])) $GLOBALS["menusub1_grid"] = new cmenusub1_grid(); // Get detail page object
				$AddRow = $GLOBALS["menusub1_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["menusub1"]->Parent->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("menusub1", $DetailTblVar)) {
				if (!isset($GLOBALS["menusub1_grid"]))
					$GLOBALS["menusub1_grid"] = new cmenusub1_grid;
				if ($GLOBALS["menusub1_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["menusub1_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["menusub1_grid"]->CurrentMode = "add";
					$GLOBALS["menusub1_grid"]->CurrentAction = "gridadd";

					// Save current master table to detail table
					$GLOBALS["menusub1_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["menusub1_grid"]->setStartRecordNumber(1);
					$GLOBALS["menusub1_grid"]->Parent->FldIsDetailKey = TRUE;
					$GLOBALS["menusub1_grid"]->Parent->CurrentValue = $this->MainMenu->CurrentValue;
					$GLOBALS["menusub1_grid"]->Parent->setSessionValue($GLOBALS["menusub1_grid"]->Parent->CurrentValue);
				}
			}
		}
	}
...
<?php if ($_menu->MainMenu->Visible) { // MainMenu ?>
	<tr id="r_MainMenu"<?php echo $_menu->RowAttributes() ?>>
		<td><span id="elh__menu_MainMenu"><?php echo $_menu->MainMenu->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $_menu->MainMenu->CellAttributes() ?>><span id="el__menu_MainMenu" class="control-group">
<input type="text" data-field="x_MainMenu" name="x_MainMenu" id="x_MainMenu" size="30" maxlength="100" placeholder="<?php echo $_menu->MainMenu->PlaceHolder ?>" value="<?php echo $_menu->MainMenu->EditValue ?>"<?php echo $_menu->MainMenu->EditAttributes() ?>>
</span><?php echo $_menu->MainMenu->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($_menu->Link->Visible) { // Link ?>
	<tr id="r_Link"<?php echo $_menu->RowAttributes() ?>>
		<td><span id="elh__menu_Link"><?php echo $_menu->Link->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $_menu->Link->CellAttributes() ?>><span id="el__menu_Link" class="control-group">
<input type="text" data-field="x_Link" name="x_Link" id="x_Link" size="30" maxlength="255" placeholder="<?php echo $_menu->Link->PlaceHolder ?>" value="<?php echo $_menu->Link->EditValue ?>"<?php echo $_menu->Link->EditAttributes() ?>>
</span><?php echo $_menu->Link->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($_menu->No->Visible) { // No ?>
	<tr id="r_No"<?php echo $_menu->RowAttributes() ?>>
		<td><span id="elh__menu_No"><?php echo $_menu->No->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $_menu->No->CellAttributes() ?>><span id="el__menu_No" class="control-group">
<input type="text" data-field="x_No" name="x_No" id="x_No" size="30" placeholder="<?php echo $_menu->No->PlaceHolder ?>" value="<?php echo $_menu->No->EditValue ?>"<?php echo $_menu->No->EditAttributes() ?>>
</span><?php echo $_menu->No->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($_menu->Akses->Visible) { // Akses ?>
	<tr id="r_Akses"<?php echo $_menu->RowAttributes() ?>>
		<td><span id="elh__menu_Akses"><?php echo $_menu->Akses->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $_menu->Akses->CellAttributes() ?>><span id="el__menu_Akses" class="control-group">
<select data-field="x_Akses" id="x_Akses[]" name="x_Akses[]" multiple="multiple"<?php echo $_menu->Akses->EditAttributes() ?>>
<?php
...
</span><?php echo $_menu->Akses->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<?php
	if (in_array("menusub1", explode(",", $_menu->getCurrentDetailTable())) && $menusub1->DetailAdd) {
?>
<?php include_once "menusub1grid.php" ?>
<?php } ?>

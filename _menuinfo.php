...
class c_menu extends cTable {
	...
	function __construct() {
		...
		// ID
		$this->ID = new cField('_menu', 'menu', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// MainMenu
		$this->MainMenu = new cField('_menu', 'menu', 'x_MainMenu', 'MainMenu', '`MainMenu`', '`MainMenu`', 200, -1, FALSE, '`MainMenu`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['MainMenu'] = &$this->MainMenu;

		// Link
		$this->Link = new cField('_menu', 'menu', 'x_Link', 'Link', '`Link`', '`Link`', 200, -1, FALSE, '`Link`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Link'] = &$this->Link;

		// No
		$this->No = new cField('_menu', 'menu', 'x_No', 'No', '`No`', '`No`', 3, -1, FALSE, '`No`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->No->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['No'] = &$this->No;

		// Akses
		$this->Akses = new cField('_menu', 'menu', 'x_Akses', 'Akses', '`Akses`', '`Akses`', 200, -1, FALSE, '`Akses`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "menusub1") {
			$sDetailUrl = $GLOBALS["menusub1"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&Parent=" . $this->MainMenu->CurrentValue;
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "_menulist.php";
		}
		return $sDetailUrl;
	}
	...
	function Update(&$rs, $where = "", $rsold = NULL) {
		...
		if (!is_null($rsold) && (isset($rsnew['MainMenu']) && $rsold['%f'] <> $rsnew['%f'])) {
			if (!isset($GLOBALS["menusub1"])) $GLOBALS["menusub1"] = new cmenusub1();
			$rscascade = array();
			$rscascade['Parent'] = $rsnew['MainMenu']; 
			$GLOBALS["menusub1"]->Update($rscascade, "`Parent` = " . ew_QuotedValue($rsold['MainMenu'], EW_DATATYPE_STRING));
		}
	...
	function Delete(&$rs, $where = "") {
		...
		// Cascade delete detail table 'menusub1'
		if (!isset($GLOBALS["menusub1"])) $GLOBALS["menusub1"] = new cmenusub1();
		$rscascade = array();
		$GLOBALS["menusub1"]->Delete($rscascade, "`Parent` = " . ew_QuotedValue($rs['MainMenu'], EW_DATATYPE_STRING));
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("_menuview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("_menuview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			...
			return $this->KeyUrl("_menuedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			...
			return $this->KeyUrl("_menuadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->MainMenu->Exportable) $Doc->ExportCaption($this->MainMenu);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->MainMenu->Exportable) $Doc->ExportField($this->MainMenu);

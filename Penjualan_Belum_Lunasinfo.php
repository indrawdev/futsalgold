...
$Penjualan_Belum_Lunas = NULL;

//
// Table class for Penjualan Belum Lunas
//
class cPenjualan_Belum_Lunas extends cTable {
	var $ID;
	var $Tgl;
	var $No_Faktur;
	var $Grand_Total;
	...
	function __construct() {
		...
		$this->TableVar = 'Penjualan_Belum_Lunas';
		$this->TableName = 'Penjualan Belum Lunas';
		...
		// ID
		$this->ID = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_ID', 'ID', '`penjualan - master`.ID', '`penjualan - master`.ID', 3, -1, FALSE, '`penjualan - master`.ID', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// Tgl
		$this->Tgl = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_Tgl', 'Tgl', '`penjualan - master`.Tgl', 'DATE_FORMAT(`penjualan - master`.Tgl, \'%d/%m/%Y\')', 133, 7, FALSE, '`penjualan - master`.Tgl', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Tgl->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['Tgl'] = &$this->Tgl;

		// No Faktur
		$this->No_Faktur = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_No_Faktur', 'No Faktur', '`penjualan - master`.`No Faktur`', '`penjualan - master`.`No Faktur`', 200, -1, FALSE, '`penjualan - master`.`No Faktur`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['No Faktur'] = &$this->No_Faktur;

		// Grand Total
		$this->Grand_Total = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_Grand_Total', 'Grand Total', '`penjualan - master`.`Grand Total`', '`penjualan - master`.`Grand Total`', 3, -1, FALSE, '`penjualan - master`.`Grand Total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Grand_Total->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Grand Total'] = &$this->Grand_Total;

		// Dibayar
		$this->Dibayar = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_Dibayar', 'Dibayar', '`penjualan - master`.Dibayar', '`penjualan - master`.Dibayar', 3, -1, FALSE, '`penjualan - master`.Dibayar', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Dibayar->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Dibayar'] = &$this->Dibayar;

		// SisaDibayar
		$this->SisaDibayar = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_SisaDibayar', 'SisaDibayar', '`penjualan - master`.SisaDibayar', '`penjualan - master`.SisaDibayar', 3, -1, FALSE, '`penjualan - master`.SisaDibayar', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->SisaDibayar->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['SisaDibayar'] = &$this->SisaDibayar;

		// Kode Pelanggan
		$this->Kode_Pelanggan = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_Kode_Pelanggan', 'Kode Pelanggan', '`penjualan - master`.`Kode Pelanggan`', '`penjualan - master`.`Kode Pelanggan`', 200, -1, FALSE, '`penjualan - master`.`Kode Pelanggan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Kode Pelanggan'] = &$this->Kode_Pelanggan;

		// Kode Kasir
		$this->Kode_Kasir = new cField('Penjualan_Belum_Lunas', 'Penjualan Belum Lunas', 'x_Kode_Kasir', 'Kode Kasir', '`penjualan - master`.`Kode Kasir`', '`penjualan - master`.`Kode Kasir`', 200, -1, FALSE, '`penjualan - master`.`Kode Kasir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function SqlSelect() { // Select
		return "SELECT `penjualan - master`.ID, `penjualan - master`.Tgl, `penjualan - master`.`No Faktur`, `penjualan - master`.`Grand Total`, `penjualan - master`.Dibayar, `penjualan - master`.SisaDibayar, `penjualan - master`.`Kode Pelanggan`, `penjualan - master`.`Kode Kasir` FROM " . $this->SqlFrom();
	}

	function SqlWhere() { // Where
		$sWhere = "`penjualan - master`.SisaDibayar > 0";
		$this->TableFilter = "";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "Penjualan_Belum_Lunaslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("Penjualan_Belum_Lunasview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("Penjualan_Belum_Lunasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "Penjualan_Belum_Lunasadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("Penjualan_Belum_Lunasedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("Penjualan_Belum_Lunasadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("Penjualan_Belum_Lunasdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
		...
		if (!ew_Empty($this->ID->CurrentValue)) {
			$this->ID->HrefValue = "penjualan_2D_detaillist.php?showmaster=penjualan_2D_master&ID=" . ((!empty($this->ID->ViewValue)) ? $this->ID->ViewValue : $this->ID->CurrentValue); // Add prefix/suffix
			$this->ID->LinkAttrs["target"] = "_blank"; // Add target
			if ($this->Export <> "") $this->ID->HrefValue = ew_ConvertFullUrl($this->ID->HrefValue);
		} else {
			$this->ID->HrefValue = "";
		}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) {
			...
			$Doc->ExportAggregate($this->Dibayar, 'TOTAL');
			$Doc->ExportAggregate($this->SisaDibayar, 'TOTAL');

...
$menusub1_view = NULL; // Initialize page object first

class cmenusub1_view extends cmenusub1 {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["ID"] <> "") {
				...
				$sReturnUrl = "menusub1list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "menusub1list.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "menusub1list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// Detail table 'menusub2'
		$body = $Language->TablePhrase("menusub2", "TblCaption");
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("menusub2list.php?" . EW_TABLE_SHOW_MASTER . "=menusub1&_Menu=" . strval($this->_Menu->CurrentValue) . "") . "\">" . $body . "</a>";
		$item = &$option->Add("detail_menusub2");
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'menusub2');
		if ($item->Visible) {
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->Akses->CurrentValue) <> "") {
				switch ($this->Akses->CurrentValue) {
					case $this->Akses->FldTagValue(1):
						$this->Akses->ViewValue = $this->Akses->FldTagCaption(1) <> "" ? $this->Akses->FldTagCaption(1) : $this->Akses->CurrentValue;
						break;
					case $this->Akses->FldTagValue(2):
						$this->Akses->ViewValue = $this->Akses->FldTagCaption(2) <> "" ? $this->Akses->FldTagCaption(2) : $this->Akses->CurrentValue;
						break;
					case $this->Akses->FldTagValue(3):
						$this->Akses->ViewValue = $this->Akses->FldTagCaption(3) <> "" ? $this->Akses->FldTagCaption(3) : $this->Akses->CurrentValue;
						break;
					default:
						$this->Akses->ViewValue = $this->Akses->CurrentValue;
				}
			} else {
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_menusub1\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_menusub1',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmenusub1view,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("menusub2", $DetailTblVar)) {
				...
				if ($GLOBALS["menusub2_grid"]->DetailView) {
					$GLOBALS["menusub2_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($menusub1_view)) $menusub1_view = new cmenusub1_view();

// Page init
$menusub1_view->Page_Init();

// Page main
$menusub1_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$menusub1_view->Page_Render();
?>
...
// Page object
var menusub1_view = new ew_Page("menusub1_view");
menusub1_view.PageID = "view"; // Page ID
var EW_PAGE_ID = menusub1_view.PageID; // For backward compatibility

// Form object
var fmenusub1view = new ew_Form("fmenusub1view");

// Form_CustomValidate event
fmenusub1view.Form_CustomValidate = 
...
fmenusub1view.ValidateRequired = true;
<?php } else { ?>
fmenusub1view.ValidateRequired = false; 
<?php } ?>
...
<?php $menusub1_view->ExportOptions->Render("body") ?>
<?php if (!$menusub1_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($menusub1_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $menusub1_view->ShowPageHeader(); ?>
<?php
$menusub1_view->ShowMessage();
?>
<form name="fmenusub1view" id="fmenusub1view" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="menusub1">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_menusub1view" class="table table-bordered table-striped">
<?php if ($menusub1->_Menu->Visible) { // Menu ?>
	<tr id="r__Menu"<?php echo $menusub1->RowAttributes() ?>>
		<td><span id="elh_menusub1__Menu"><?php echo $menusub1->_Menu->FldCaption() ?></span></td>
...
<?php if ($menusub1->Link->Visible) { // Link ?>
	<tr id="r_Link"<?php echo $menusub1->RowAttributes() ?>>
		<td><span id="elh_menusub1_Link"><?php echo $menusub1->Link->FldCaption() ?></span></td>
...
<?php if ($menusub1->Parent->Visible) { // Parent ?>
	<tr id="r_Parent"<?php echo $menusub1->RowAttributes() ?>>
		<td><span id="elh_menusub1_Parent"><?php echo $menusub1->Parent->FldCaption() ?></span></td>
...
<?php if ($menusub1->No->Visible) { // No ?>
	<tr id="r_No"<?php echo $menusub1->RowAttributes() ?>>
		<td><span id="elh_menusub1_No"><?php echo $menusub1->No->FldCaption() ?></span></td>
...
<?php if ($menusub1->Akses->Visible) { // Akses ?>
	<tr id="r_Akses"<?php echo $menusub1->RowAttributes() ?>>
		<td><span id="elh_menusub1_Akses"><?php echo $menusub1->Akses->FldCaption() ?></span></td>
...
<?php
	if (in_array("menusub2", explode(",", $menusub1->getCurrentDetailTable())) && $menusub2->DetailView) {
?>
